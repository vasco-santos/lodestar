...
export class CachedValidatorsBeaconState {
  ...
  public createProxy(): CachedValidatorsBeaconState {
    return new Proxy(this, new StateTransitionBeaconStateProxyHandler());
  }
  ...
  public addValidator(validator: Validator): void {
    if (this.isSynced) {
      this._cachedValidators.push(validator);
    }
  ...
  /**
   * Warning: only clone the original BeaconState, share same validators cache.
   */
...
class StateTransitionBeaconStateProxyHandler implements ProxyHandler<CachedValidatorsBeaconState> {
  /**
